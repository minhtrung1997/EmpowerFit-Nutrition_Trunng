...
// ============================================================
// PRINT DIARY (full meal record)
// ============================================================
function printDiary() {
  if (!state.client.name || !state.targets) { alert('Fill client details first.'); return; }
  ...
  MEAL_SLOTS.forEach(slot => {
    ...
    mealsHtml += '<table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background:#dceadc;"><th style="padding:4px 6px;text-align:left">Food</th><th>Amount</th><th>kcal</th><th>Pro</th><th>Carbs</th><th>Fat</th></tr></thead><tbody>' + rows + '</tbody></table>';
  });
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Diary - ' + name + '</title><style>body{font-family:Arial,sans-serif;font-size:13px;padding:30px;max-width:800px;margin:auto}h1{color:#2e7d32;font-size:20px}h2{font-size:14px;margin:16px 0 6px;color:#1a2e1a}table{margin-bottom:8px}td,th{padding:4px 6px;border-bottom:1px solid #ccc}@media print{body{-webkit-print-color-adjust:exact}}</style></head><body>' +
    ...
    '<p><strong>Date:</strong> ' + date + ' | <strong>Weight:</strong> ' + weight + 'kg | <strong>Height:</strong> ' + height + 'cm | <strong>Age:</strong> ' + age + ' | <strong>Sex:</strong> ' + sex + '</p>' +
    '<p><strong>Activity:</strong> ' + activity + ' | <strong>Goal:</strong> ' + goal + '</p>' +
    '<p><strong>Target Formula:</strong> ' + mode + ' | <strong>Food Mode:</strong> ' + calibMode + ' | <strong>Coach Factors:</strong> ' + factors + '</p>' +
    '<h2>Targets: ' + targets.calories + ' kcal | Pro: ' + targets.protein + 'g | Carbs: ' + targets.carbs + 'g | Fat: ' + targets.fat + 'g</h2>' +
    '<hr>' + mealsHtml +
    '<hr><h2>Day Total: ' + formatMacro(totals.calories) + ' kcal | Pro: ' + formatMacro(totals.protein) + 'g | Carbs: ' + formatMacro(totals.carbs) + 'g | Fat: ' + formatMacro(totals.fat) + 'g</h2>' +
    '<p style="font-size:10px;color:#888;margin-top:20px">Generated by House of Health Well Pro — Nutrition Planner</p></body></html>';
...
// ============================================================
// PRINT CALCULATION LOG (formulae breakdown)
// ============================================================
function printCalcLog() {
  if (!state.client.weight || !state.targets) { alert('Fill client details first.'); return; }
  const { name, weight, height, age, sex, activity, goal } = state.client;
  const bmr = calculateBMR(weight, height, age, sex);
  const tdee = calculateTDEE(bmr, activity);
  const mult = ACTIVITY_MULTIPLIERS[activity];
  const targets = state.targets;
  const mode = state.targetFormula;
  const calibMode = state.calibrationMode;

  let log = '<h1>📐 Calculation Log — ' + (name || 'Client') + '</h1>';
  log += '<h2>Step 1: BMR (Mifflin-St Jeor)</h2>';
  if (sex === 'male') {
    log += '<p>BMR = 10 × ' + weight + ' + 6.25 × ' + height + ' - 5 × ' + age + ' + 5 = <strong>' + Math.round(bmr) + ' kcal</strong></p>';
  } else {
    log += '<p>BMR = 10 × ' + weight + ' + 6.25 × ' + height + ' - 5 × ' + age + ' - 161 = <strong>' + Math.round(bmr) + ' kcal</strong></p>';
  }

  log += '<h2>Step 2: TDEE</h2>';
  log += '<p>TDEE = BMR × ' + mult + ' (' + activity + ') = <strong>' + tdee + ' kcal</strong></p>';

  log += '<h2>Step 3: Target Calculation (' + (mode === 'herbalife' ? 'Herbalife Coach' : 'Normal') + ')</h2>';
  if (mode === 'herbalife') {
    var goalMap = {'Fat Loss (Aggressive)':26,'Fat Loss (Moderate)':28,'Maintenance':30,'Recomp':30,'Lean Bulk':34,'Muscle Gain (Moderate)':36,'Athletic Performance':32,'Endurance':30,'Enhanced Athlete':38};
    var kcalKg = goalMap[goal] || 30;
    log += '<p>Calories = ' + weight + ' kg × ' + kcalKg + ' kcal/kg = <strong>' + Math.round(weight * kcalKg) + ' kcal</strong></p>';
    log += '<p>Protein = ' + weight + ' kg × 1.75 g/kg = <strong>' + Math.round(weight * 1.75) + 'g</strong></p>';
    log += '<p>Fat = ' + weight + ' kg × 0.7 g/kg = <strong>' + Math.round(weight * 0.7) + 'g</strong></p>';
  } else {
    var goalAdj = GOALS[goal] ? GOALS[goal].adjustment : 0;
    log += '<p>Calories = TDEE + goal adjustment (' + goalAdj + ') = ' + tdee + ' + ' + goalAdj + ' = <strong>' + (tdee + goalAdj) + ' kcal</strong></p>';
    log += '<p>Protein = ' + weight + ' kg × 2.2 g/kg = <strong>' + Math.round(weight * 2.2) + 'g</strong></p>';
    log += '<p>Fat = ' + weight + ' kg × 0.8 g/kg = <strong>' + Math.round(weight * 0.8) + 'g</strong></p>';
  }
  ...
  // Per-meal breakdown
  log += '<h2>Meal Breakdown</h2>';
  MEAL_SLOTS.forEach(function(slot) {
    var items = state.meals[slot.id];
    if (items.length === 0) return;
  ...
  log += '<p style="margin-top:20px;font-size:10px;color:#888">Calibration mode: ' + calibMode + ' | Target formula: ' + (mode === 'herbalife' ? 'Herbalife Coach' : 'Normal') + '</p>';

  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Calculation Log</title><style>body{font-family:Arial,sans-serif;font-size:13px;padding:30px;max-width:800px;margin:auto}h1{color:#2e7d32}h2{color:#1b5e20;font-size:14px;margin:16px 0 6px;border-bottom:1px solid #ccc;padding-bottom:4px}h3{font-size:13px;margin:8px 0 2px}p{margin:4px 0}@media print{body{-webkit-print-color-adjust:exact}}</style></head><body>' + log + '</body></html>';
